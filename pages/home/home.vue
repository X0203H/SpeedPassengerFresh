<template>
	<view class="content">
		...
		<view class="recommend">
			<navigator class='item' url="/pages/classify/classify" open-type="switchTab" v-for="item in store.state.meun"
				:key="item.id">
			...
			<view class="title">
				...
				<view class="endTime">
					...
					<van-count-down use-slot :time="time" @change="onChange">
						<text class="item">{{ timeData.hours>=10?timeData.hours:('0'+timeData.hours) }}</text> :
						<text class="item"> {{ timeData.minutes>=10?timeData.minutes:('0'+timeData.minutes) }}</text> :
						<text class="item"> {{ timeData.seconds>=10?timeData.seconds:('0'+timeData.seconds) }}</text>
					</van-count-down>
				</view>
			</view>
			<scroll-view class="list" :scroll-x="true">
				<navigator url='/pages/classify/classify' open-type="switchTab" class="items" v-for="item in flashData"
					:key='item.id'>
		...
		<view class="flash_sale">
			...
			<scroll-view class="list" :scroll-x="true">
				<navigator url='/pages/classify/classify' open-type="switchTab" class="items" v-for="item in newComerList"
					:key='item.id'>
					<image :src="item.coverImage" />
					<view class="qianggo">限时抢购</view>
					<view class="name">{{item.name}}</view>
